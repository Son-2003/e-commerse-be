...
public class JwtTokenProvider {
    ...
    public boolean validateToken(String token) {
        if (token == null || token.isEmpty()) {
            throw new ECommerseException(HttpStatus.BAD_REQUEST, "JWT token is empty or null");
        }else{
            try {
                Jwts.parserBuilder()
                        .setSigningKey(key())
                ...
                throw new ECommerseException(HttpStatus.BAD_REQUEST, "Invalid JWT token");
            } catch (ExpiredJwtException e) {
                throw new ECommerseException(HttpStatus.BAD_REQUEST, "Expired JWT token");
            } catch (UnsupportedJwtException e) {
                throw new ECommerseException(HttpStatus.BAD_REQUEST, "Unsupported JWT token");
            } catch (IllegalArgumentException e) {
                throw new ECommerseException(HttpStatus.BAD_REQUEST, "JWT claims string is empty");
            }
        }
    }
